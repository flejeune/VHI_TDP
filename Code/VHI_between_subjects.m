...
%%
function i_fig = VHI_between_subjects( subj, folder_path, i_fig)
...
%-------------------------------SAVE DATA---------------------------------%
save([folder_path 'BetweenSubjects_TDPT' num2str(s) '.mat'],'numb_avams_m', 'ca_m');

save([folder_path 'BetweenSubjects_TDPTmean' num2str(s) '.mat'],'mean_numb_avams', 'mean_ca');
...
for s = 1:length(subj)
    ...
    elseif s<25
        ...
        title(['Subj ' num2str(s) ' - ' subj{s}(4:end)])
        if s==length(subj)
            leg_due = legend('1st - Pre','2nd - Post');
...
savefig(f1, [folder_path '46-BetweenSubjects_individualTDPT1.fig']);
savefig(f2, [folder_path '47-BetweenSubjects_individualTDPT2.fig']);
savefig(f3, [folder_path '48-BetweenSubjects_individualTDPT3.fig']);
savefig(f4, [folder_path '49-BetweenSubjects_individualTDPT4.fig']);
savefig(f5, [folder_path '50-BetweenSubjects_individualTDPT5.fig']);
savefig(f6, [folder_path '51-BetweenSubjects_individualTDPT6.fig']);
...
savefig(f, [folder_path '51-BetweenSubjects_averageTDPT.fig']);

f = figure(i_fig); i_fig = i_fig + 1;
temp(:,:) = 100*sum(numb_avams_m, 1)/48;
avamsMean = mean(temp,2);
avamsStd = std(temp,0,2)/sqrt(s);
X = categorical({'1st - Pre';'2nd - Post'});
colori = 'kbgr';
hold on
for k = 1:2
...
savefig(f, [folder_path '52-BetweenSubjects_forearm.fig']);
