...
%%
function i_fig = VHI_fitting(s, folder_path, i_fig)
%VHI_fitting Performs fitting on TDPT individual data
%   Performs fitting on individual TDPT data
%   Computes PSE & EA
%   Plots results
...
% a = 0;
% b = 10;
% y = 100*1./(1+exp(-(x-a)/(0.5*b)));
% cftool(x, y);

% In the opened window, instead of 'Polynomial' select 'Custom Equation'
% In the text box under f(x), copy the following equation: 
% 100*1/(1+exp(-(x-a)/(0.5*b)))
% Then click 'File' > 'Generate Code', save the scrit with the same name as
% the function (createFit.m)
...
% Save results
save([folder_path 'fitResults' num2str(s) '_createFit.mat'], 'x', 'fitResults', 'coefficients')
...
% Save results
save([folder_path 'fitOfAverage' num2str(s) '.mat'], 'fitobjectPre', 'fitobject20A', 'fitobject20S', 'fitobject40S',...
    'gofPre', 'gof20A', 'gof20S', 'gof40S', ...
    'coeffsPre', 'coeffs20S', 'coeffs40S', 'coeffs20A');

%% PSE_delta. For each subject, compute:  PSE 20S - PSE Pre, PSE 40S - PSE Pre, PSE 20A - PSE Pre
for i=1:length(subj)
    for j = 2:4
        PSE_delta(i,j-1) = coefficients(j,1,i) - coefficients(1,1,i);
    end
end
...
for i=1:length(subj)
    if i<13
        ...
        if i==12
            leg_due = legend('Pre', 'SigPre', '20cmS', 'Sig20S','40cmS', 'Sig40S','20cmA', 'Sig20A');
...
savefig(f1, [folder_path '9-individualFit1.fig']);
savefig(f2, [folder_path '10-individualFit2.fig']);
savefig(f3, [folder_path '11-individualFit3.fig']);
savefig(f4, [folder_path '12-individualFit4.fig']);
savefig(f5, [folder_path '13-individualFit5.fig']);
savefig(f6, [folder_path '14-individualFit6.fig']);
...
legend('Pre', 'SigPre', '20cmS', 'Sig20S','40cmS', 'Sig40S','20cmA', 'Sig20A');
xlabel('Diff. of the stimulation distance between the forearm and forehead [mm]')
ylabel('Perc. of "forearm" responses')
title({['Fit Of Averaged Data (over ' num2str(length(subj)) ' subjects)']; 'Forearm answers - SPOT OF STIMULUS'})
ylim([0 100]);
savefig(f, [folder_path '15-fit_of_averages.fig']);
...
savefig(f, [folder_path '16-PSE_EA.fig']);
